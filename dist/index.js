...
import { resolveColorFn, formatInputTokens } from "./utils/style.ts";
import { wrapTextToRows, getContentWidth, getPhysicalPos, getCursorFromPhysicalPos } from "./utils/layout.ts";
export * from "./types.ts";
export default function TextInput(prefixOrOptions = "❯", options = {}) {
    let prefixText = "❯";
    let opts = {};
    ...
    const { required = true, prefixColor, placeholder = "Enter your message or @path/to/file", history = [], MAX_HISTORY_SIZE = 10, styles = {}, } = opts;
    const { backgroundColor = "#121212", colorPrefixBg = true, paddingTop = 1, paddingBottom = 1, paddingRight = 1, marginVertical = 1, marginHorizontal = 0, wordHighlight = {}, } = styles;
    ...
    return new Promise((resolve) => {
        ...
        function renderFullWidthLine(prefixContent, bodyContent, cols) {
        ...
        function render() {
            ...
            for (let m = 0; m < marginVertical; m++)
                formattedLines.push("");
            for (let p = 0; p < paddingTop; p++)
                formattedLines.push(renderFullWidthLine("", "", cols));
            if (!value) {
                ...
                for (let l = 0; l < logicalLines.length; l++) {
                    ...
                    for (let s = 0; s < subRows.length; s++) {
                        const formattedSubRow = formatInputTokens(highlightPrefix, subRows[s], highlightColor);
        ...
        function handleKey(key) {
            ...
            if (key === "\u001b[A") {
                if (contentRow > 0) {
                    cursor = getCursorFromPhysicalPos(contentRow - 1, pos.col, cols, value, prefixWidth, styles);
                    render();
                }
            ...
            // Arrow down key
            if (key === "\u001b[B") {
                const totalContentRows = pos.totalRows - marginVertical * 2 - paddingTop - paddingBottom;
